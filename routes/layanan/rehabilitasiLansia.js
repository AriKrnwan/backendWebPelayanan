...
router.post('/upload-Rehabilitasi-Lansia', authenticateUser, upload.fields([
    { name: 'ktp' },
    { name: 'kk' },
]), async (req, res) => {
    ...
    try {
        ...
        const [countResult] = await db.execute(
            'SELECT COUNT(*) as count FROM lay_rehab_lansia WHERE DATE(submit_at) = ?',
            [formattedDate]
        );
        ...
        while (!isUnique) {
            no_lay = `RSL${formattedDate}${count.toString().padStart(3, '0')}`;

            const [existingNoLay] = await db.execute(
                'SELECT COUNT(*) as count FROM lay_rehab_lansia WHERE no_lay = ?',
                [no_lay]
            );
        ...
        const [result] = await db.execute(
            'INSERT INTO lay_rehab_lansia (no_lay, user_nik, ktp, kk, kebutuhan, submit_at) VALUES (?, ?, ?, ?, ?, NOW())',
            [
                no_lay,
                user_nik,
...
router.get('/lay-rehabilitasi-lansia', async (req, res) => {
    const { user_nik } = req.query;

    try {
        const [rows] = await db.execute(
            'SELECT id, no_lay, user_nik, ktp, kk, kebutuhan, submit_at, valid_at, reject_at, accept_at FROM lay_rehab_lansia WHERE user_nik = ?',
            [user_nik]
        );
...
router.get('/lay-rehabilitasi-lansia/:nopel', authenticateUser, async (req, res) => {
    const { nopel } = req.params;

    try {
        const [rows] = await db.execute(
            'SELECT * FROM lay_rehab_lansia WHERE no_lay = ?',
            [nopel]
        );
        ...
        const rehabLansia = rows[0];
        const userId = rehabLansia.user_nik;
...
router.put('/update-rehabilitasi-lansia/:no_lay', authenticateUser, upload.fields([
    { name: 'ktp' },
    { name: 'kk' },
    { name: 'product' }
]), async (req, res) => {
...
router.delete('/delete-rehabilitasi-lansia/:no_lay', authenticateUser, async (req, res) => {
    const { no_lay } = req.params;
    ...
    try {
        ...
        await db.execute(
            'DELETE FROM lay_rehab_lansia WHERE no_lay = ?',
            [no_lay]
        );
...
router.post('/admin-upload-rehabilitasi-lansia', authenticateUser, upload.fields([
    { name: 'ktp' },
    { name: 'kk' },
]), async (req, res) => {
